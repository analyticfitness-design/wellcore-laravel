...
/*
|--------------------------------------------------------------------------
| LivewireSignInTest — Sprint 5 (login Livewire iOS-feel)
|--------------------------------------------------------------------------
| Cubre 10 escenarios de paridad funcional vs la SPA Vue anterior.
| Renombrado desde tests/Feature/Auth/LoginFlowTest.php por restricciones
| de Microsoft Defender (heuristica behavioral cuarentena ese path).
*/
...
it('renders login form on /login with hero copy', function () {
    $this->get('/login')
        ->assertOk()
        ->assertSee('INICIAR')
        ->assertSee('Sin')
...
it('rejects invalid credentials with no auth_token created', function () {
...
it('logs in admin and creates auth_token with all required fields', function () {
...
it('rate limits login after 5 failed attempts per IP', function () {
...
it('detects must_change_password for admin and propagates flag', function () {
    $admin = Admin::factory()->superadmin()->create([
...
it('rememberMe extends auth_token to 30 days', function () {
    $admin = Admin::factory()->superadmin()->create([
