<div class="space-y-6" wire:poll.10s>

    {{-- Header --}}
    <div>
        <h1 class="font-display text-3xl tracking-wide text-wc-text sm:text-4xl">Mensajes</h1>
        <p class="mt-1 text-sm text-wc-text-tertiary">Comunicacion con tus clientes</p>
    </div>

    {{-- Message center layout --}}
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-12" style="min-height: 70vh;">

        {{-- Client list panel --}}
        <div class="rounded-card border border-wc-border bg-wc-bg-tertiary lg:col-span-4 overflow-hidden flex flex-col">
            <div class="border-b border-wc-border px-4 py-3">
                <p class="text-xs font-semibold uppercase tracking-wider text-wc-text-tertiary">Clientes</p>
            </div>

            <div class="flex-1 overflow-y-auto">
                @if($clients->count() > 0)
                    <ul class="divide-y divide-wc-border">
                        @foreach($clients as $client)
                            <li>
                                <button
                                    wire:click="selectClient({{ $client['id'] }})"
                                    class="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors
                                           {{ $selectedClientId === $client['id'] ? 'bg-wc-accent/5 border-l-2 border-l-wc-accent' : 'hover:bg-wc-bg-secondary/50' }}"
                                >
                                    {{-- Avatar with unread indicator --}}
                                    <div class="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-wc-accent/15">
                                        <span class="text-sm font-semibold text-wc-accent">{{ $client['initial'] }}</span>
                                        @if($client['unread_count'] > 0)
                                            <div class="absolute -right-0.5 -top-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-wc-accent text-[9px] font-bold text-white">
                                                {{ $client['unread_count'] > 9 ? '9+' : $client['unread_count'] }}
                                            </div>
                                        @endif
                                    </div>

                                    {{-- Name + preview --}}
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center justify-between gap-2">
                                            <p class="text-sm font-medium text-wc-text truncate {{ $client['unread_count'] > 0 ? 'font-semibold' : '' }}">
                                                {{ $client['name'] }}
                                            </p>
                                            @if($client['last_message_time'])
                                                <span class="shrink-0 text-[10px] text-wc-text-tertiary">{{ $client['last_message_time'] }}</span>
                                            @endif
                                        </div>
                                        <p class="text-xs text-wc-text-tertiary truncate {{ $client['unread_count'] > 0 ? 'text-wc-text-secondary font-medium' : '' }}">
                                            {{ $client['last_message_preview'] }}
                                        </p>
                                    </div>
                                </button>
                            </li>
                        @endforeach
                    </ul>
                @else
                    <div class="flex flex-col items-center justify-center py-12 text-center px-4">
                        <svg class="h-8 w-8 text-wc-text-tertiary" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
                        </svg>
                        <p class="mt-2 text-sm text-wc-text-tertiary">Sin clientes asignados</p>
                    </div>
                @endif
            </div>
        </div>

        {{-- Conversation panel --}}
        <div class="rounded-card border border-wc-border bg-wc-bg-tertiary lg:col-span-8 overflow-hidden flex flex-col">
            @if($selectedClient)
                {{-- Conversation header --}}
                <div class="flex items-center gap-3 border-b border-wc-border px-4 py-3">
                    <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-wc-accent/15">
                        <span class="text-sm font-semibold text-wc-accent">{{ substr($selectedClient->name ?? 'C', 0, 1) }}</span>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-wc-text">{{ $selectedClient->name ?? 'Cliente' }}</p>
                        <p class="text-xs text-wc-text-tertiary">{{ $selectedClient->plan?->label() ?? 'Sin plan' }}</p>
                    </div>
                </div>

                {{-- Messages --}}
                <div class="flex-1 overflow-y-auto px-4 py-4 space-y-3" style="max-height: 55vh;">
                    @if($messages->count() > 0)
                        @foreach($messages as $msg)
                            <div class="flex {{ $msg['is_coach'] ? 'justify-end' : 'justify-start' }}">
                                <div class="max-w-[75%] rounded-xl px-4 py-2.5
                                            {{ $msg['is_coach']
                                                ? 'bg-wc-accent text-white rounded-br-sm'
                                                : 'bg-wc-bg-secondary text-wc-text rounded-bl-sm' }}">
                                    <p class="text-sm leading-relaxed">{{ $msg['message'] }}</p>
                                    <p class="mt-1 text-[10px] {{ $msg['is_coach'] ? 'text-white/60' : 'text-wc-text-tertiary' }}">
                                        {{ $msg['time'] }}
                                    </p>
                                </div>
                            </div>
                        @endforeach
                    @else
                        <div class="flex flex-col items-center justify-center py-12 text-center">
                            <svg class="h-8 w-8 text-wc-text-tertiary" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
                            </svg>
                            <p class="mt-2 text-sm text-wc-text-tertiary">Inicia la conversacion</p>
                        </div>
                    @endif
                </div>

                {{-- Message input --}}
                <div class="border-t border-wc-border px-4 py-3">
                    <form wire:submit="sendMessage" class="flex items-center gap-2">
                        <x-template-selector context="message" target="newMessage" position="top" />
                        <input
                            type="text"
                            wire:model="newMessage"
                            placeholder="Escribe un mensaje..."
                            class="flex-1 rounded-lg border border-wc-border bg-wc-bg-secondary py-2.5 px-4 text-sm text-wc-text placeholder-wc-text-tertiary focus:border-wc-accent focus:outline-none focus:ring-1 focus:ring-wc-accent"
                        >
                        <button
                            type="submit"
                            class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-wc-accent text-white hover:bg-wc-accent-hover transition-colors"
                        >
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5" />
                            </svg>
                        </button>
                    </form>
                </div>
            @else
                {{-- No client selected --}}
                <div class="flex flex-1 flex-col items-center justify-center py-12 text-center">
                    <div class="flex h-16 w-16 items-center justify-center rounded-full bg-wc-bg-secondary">
                        <svg class="h-8 w-8 text-wc-text-tertiary" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
                        </svg>
                    </div>
                    <p class="mt-4 text-sm font-medium text-wc-text">Selecciona un cliente</p>
                    <p class="mt-1 text-xs text-wc-text-tertiary">Elige un cliente del panel izquierdo para ver la conversacion</p>
                </div>
            @endif
        </div>
    </div>

</div>
