{{--
    wire:poll.30s.visible — polling fires every 30 s AND only while the
    component is actually visible in the viewport.  When the user scrolls
    away or the tab is hidden Livewire pauses the interval automatically,
    cutting idle DB load to zero.
--}}
<div wire:poll.30s.visible="pollMessages">
    {{-- Header --}}
    <div class="mb-6">
    ...
    <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary overflow-hidden">
        ...
        <div class="flex items-center gap-3 border-b border-wc-border px-5 py-3">
            ...
            <div class="flex-1">
                ...
                @else
                    <span class="text-xs text-wc-text-tertiary/70">Sin coach asignado</span>
                @endif
            </div>
        </div>

        @if(! $hasCoach)
            {{-- No coach assigned — friendly placeholder, no chat UI --}}
        ...
        @else
            {{-- Messages Area — only rendered when there IS a real coach --}}
            <div
                x-data="{
                    scrollToBottom() {
                        this.$nextTick(() => {
            ...
            {{-- Input Area — disabled/hidden when there is no coach --}}
