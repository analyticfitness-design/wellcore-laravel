                        ...
                        <tr class="transition-colors hover:bg-wc-bg-secondary/50" wire:key="coach-{{ $admin->id }}">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    @endif
                                    {{-- Eliminar (solo superadmin, no se puede eliminar superadmin ni a sí mismo) --}}
                                    @if($isSuperAdmin && $roleVal !== 'superadmin' && $admin->id !== auth('wellcore')->id())
                                        <button wire:click="deleteCoach({{ $admin->id }})"
                                                wire:confirm="¿Eliminar a {{ $admin->name ?? $admin->username }}? Esta acción no se puede deshacer."
