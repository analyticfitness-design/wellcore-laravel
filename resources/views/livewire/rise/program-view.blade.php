<div class="space-y-6">
    ...
    @if($hasProgram)
        ...
        <div class="relative overflow-hidden rounded-card border border-amber-500/20 bg-gradient-to-br from-amber-500/8 via-amber-400/4 to-transparent p-5 sm:p-6">
            {{-- Decorative orbs --}}
            <div class="pointer-events-none absolute -right-8 -top-8 h-32 w-32 rounded-full bg-amber-500/6"></div>
            <div class="pointer-events-none absolute -right-3 -top-3 h-16 w-16 rounded-full bg-amber-500/10"></div>

            <div class="relative">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div class="flex-1">
                        <div class="flex items-center gap-2">
                            <span class="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-amber-500/15 to-amber-400/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-500">
                        ...
                        <div class="mt-3 flex flex-wrap gap-2">
                            ...
                            @if($trainingPlan && isset($trainingPlan['frecuencia']))
                                <span class="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2.5 py-1 text-xs font-medium text-amber-500">
                    ...
                    <div class="flex shrink-0 flex-col items-end">
                        <div class="flex items-baseline gap-1">
                            <span class="font-data text-4xl font-bold tabular-nums text-amber-500">{{ $currentWeek }}</span>
                ...
                <div class="mt-5">
                    <div class="flex items-center justify-between text-[11px] text-wc-text-tertiary mb-1.5">
                        ...
                        <span class="font-data font-semibold text-amber-500">{{ number_format($progressPct, 0) }}%</span>
                    </div>
        ...
        <div class="rounded-card border border-wc-border bg-wc-bg-tertiary p-1" role="tablist" aria-label="Secciones del programa">
            <div class="grid grid-cols-3 gap-1">
                ...
                <button
                    ...
                    class="flex items-center justify-center gap-2 rounded-lg px-3 py-2.5 text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-1
                        {{ $activeTab === 'training'
            ...
            <div id="panel-training" role="tabpanel" aria-labelledby="tab-training" class="space-y-4">
                ...
                @if($trainingPlan)
                    ...
                    @if(isset($trainingPlan['objetivo']))
                        <div class="flex items-start gap-3 rounded-card border border-amber-500/20 bg-amber-500/5 p-4">
                            ...
                            <div>
                                <p class="text-[10px] font-semibold uppercase tracking-wider text-amber-500/70">Objetivo del plan</p>
                        ...
                        <div class="space-y-3">
                            @foreach($trainingPlan['semanas'] as $semana)
                                ...
                                <div
                                    ...
                                    class="overflow-hidden rounded-card border transition-colors {{ $isCurrentWeek ? 'border-amber-500/30 bg-amber-500/5' : 'border-wc-border bg-wc-bg-tertiary' }}"
                                >
                                    {{-- Week header --}}
                                    <button
                                        x-on:click="open = !open"
                                        class="flex w-full items-center justify-between px-4 py-4 text-left transition-colors hover:bg-black/5 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-amber-500"
                                    ...
                                    >
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg font-data text-sm font-bold
                                                {{ $isCurrentWeek ? 'bg-amber-500 text-white' : 'bg-wc-bg-secondary text-wc-text-tertiary' }}">
                                                {{ $weekNum }}
                                            </div>
                                            ...
                                            <div>
                                                <div class="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    @if($isCurrentWeek)
                                                        <span class="rounded-full bg-amber-500 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white">Semana actual</span>
                                                    @endif
                                                    @if($fase)
                                                        ...
                                                        <div class="divide-y divide-wc-border/40 border-t border-wc-border/40">
                                                            @foreach($ejercicios as $ejercicio)
                                                                <div class="flex items-start gap-3 px-4 py-3">
                                                                    {{-- Exercise number --}}
                                                                    <span class="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-amber-500/10 font-data text-[11px] font-bold text-amber-500">
                                                                        {{ $loop->iteration }}
                                                                    </span>
                ...
                @else
                    ...
                    <div class="rounded-card border border-wc-border bg-wc-bg-tertiary px-6 py-12 text-center">
                        <div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-500/10">
                            <svg class="h-7 w-7 text-amber-500/60" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 12h16.5m-16.5 0a1.5 1.5 0 0 1-1.5-1.5v-3a1.5 1.5 0 0 1 1.5-1.5h1.5a1.5 1.5 0 0 1 1.5 1.5v3m-4.5 0a1.5 1.5 0 0 0-1.5 1.5v3a1.5 1.5 0 0 0 1.5 1.5h1.5a1.5 1.5 0 0 0 1.5-1.5v-3m12-4.5v3a1.5 1.5 0 0 1-1.5 1.5h-1.5m3 0a1.5 1.5 0 0 1-1.5 1.5v3a1.5 1.5 0 0 1-1.5 1.5h-1.5a1.5 1.5 0 0 1-1.5-1.5v-3m0-4.5a1.5 1.5 0 0 0-1.5-1.5h-1.5a1.5 1.5 0 0 0-1.5 1.5v3" />
                            </svg>
                        </div>
                            ...
                            <ul class="mt-4 space-y-2.5">
                                @foreach($nutritionPlan['tips'] as $tip)
                                    <li class="flex items-start gap-3">
                                        <div class="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-amber-500/15">
                                            <svg class="h-3 w-3 text-amber-500" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                                ...
                                @foreach($nutritionPlan['comidas_sugeridas'] as $comida)
                                    @php
                                        ...
                                        $iconBgClass = match(true) {
                                            str_contains($nombreLower, 'desayuno')        => 'bg-amber-500/10 text-amber-400',
                                    ...
                                    >
                                        <button
                                            x-on:click="open = !open"
                                            class="flex w-full items-center gap-3 p-4 text-left transition hover:bg-black/5 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-amber-500"
                                            :aria-expanded="open ? 'true' : 'false'"
                                        >
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach($habitsPlan as $habito)
                            @php
                                ...
                                $accentColors = [
                                    'amber'   => ['border' => 'border-amber-500/25',   'bg' => 'bg-amber-500/5',    'icon_bg' => 'bg-amber-500/10',    'icon_text' => 'text-amber-500',    'badge' => 'bg-amber-500/10 text-amber-500'],
    ...
    @else
        {{-- ─── NO ACTIVE PROGRAM ───────────────────────────────────────────── --}}
        <div class="rounded-card border border-amber-500/20 bg-amber-500/5 px-6 py-12 text-center">
            <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-amber-500/10">
                <svg class="h-8 w-8 text-amber-500/50" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
