<div class="space-y-6">
    ...
    <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-5 sm:p-6">
        <div class="flex items-center gap-4">
            <div class="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-amber-500/30 to-amber-600/20">
                <span class="font-display text-2xl text-amber-500">{{ $initial }}</span>
            </div>
            <div class="flex-1 min-w-0">
                <h2 class="text-lg font-semibold text-wc-text truncate">{{ $name }}</h2>
                <p class="text-sm text-wc-text-tertiary truncate">{{ $email }}</p>
                @if($status)
                    @php
                    ...
                    @endphp
                    <span class="mt-1 inline-flex items-center gap-1 rounded-full {{ $statusColor }} px-2.5 py-0.5 text-xs font-medium">
    ...
    <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-5">
        <div class="flex items-center justify-between">
            ...
            <span class="font-data text-2xl font-bold text-amber-500">{{ $progressPercent }}%</span>
        </div>
        ...
        <div class="mt-2 flex items-center justify-between text-xs text-wc-text-tertiary">
            <span>Dia {{ $daysInProgram }} de {{ $totalDays }}</span>
            <span>{{ $totalDays - $daysInProgram > 0 ? ($totalDays - $daysInProgram) . ' dias restantes' : 'Programa finalizado' }}</span>
        </div>
    </div>
        ...
        <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div class="flex items-center gap-3 rounded-lg border border-wc-border bg-wc-bg-secondary p-3">
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-wc-text">{{ $startDate ?? 'No definida' }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 rounded-lg border border-wc-border bg-wc-bg-secondary p-3">
                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-wc-accent/15">
                    <svg class="h-4.5 w-4.5 text-wc-accent" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                    </svg>
                </div>
                <div>
                    <p class="text-[10px] uppercase tracking-wider text-wc-text-tertiary">Fecha fin</p>
                    <p class="text-sm font-medium text-wc-text">{{ $endDate ?? 'No definida' }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 rounded-lg border border-wc-border bg-wc-bg-secondary p-3">
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-wc-text">{{ $experienceLevel ? ucfirst($experienceLevel) : 'No definido' }}</p>
                </div>
            </div>

            {{-- Ubicacion entrenamiento --}}
            <div class="flex items-center gap-3 rounded-lg border border-wc-border bg-wc-bg-secondary p-3">
                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-sky-500/15">
                    <svg class="h-4.5 w-4.5 text-sky-500" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-wc-text">{{ $trainingLocation ? ucfirst($trainingLocation) : 'No definida' }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 rounded-lg border border-wc-border bg-wc-bg-secondary p-3">
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-wc-text">{{ ucfirst($gender) }}</p>
                </div>
            </div>
            @endif
        </div>
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-4 text-center">
            <div class="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-amber-500/15">
                <svg class="h-5 w-5 text-amber-500" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                </svg>
            </div>
            <p class="mt-2 font-data text-2xl font-bold text-wc-text">{{ $daysInProgram }}</p>
        ...
        <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-4 text-center">
            ...
            <p class="mt-2 font-data text-2xl font-bold text-wc-text">{{ $measurementCount }}</p>
        ...
        <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-4 text-center">
            ...
            <p class="mt-2 font-data text-2xl font-bold text-wc-text">{{ $checkinsCount }}</p>
        ...
        <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-4 text-center">
            ...
            <p class="mt-2 font-data text-2xl font-bold text-wc-text">{{ $adherence }}%</p>
    ...
    <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-5">
        ...
        <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <a href="{{ route('rise.tracking') }}"
               class="flex items-center gap-3 rounded-lg border border-wc-border bg-wc-bg-secondary p-3 transition-colors hover:border-amber-500/30 hover:bg-amber-500/5">
                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-500/15">
                    <svg class="h-4.5 w-4.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                    </svg>
