...
// Items del drawer "Mas" del bottom nav: todos los modulos del sidebar
// excepto los 5 fixedTabs (Dashboard, Clientes, Pagos, Feed, [Mas]).
// Se computa desde navSections para mantener una unica fuente de verdad.
const FIXED_TAB_ROUTES = new Set(['admin-dashboard', 'admin-clients', 'admin-payments', 'admin-feed']);
const drawerItems = computed(() => {
    const result = [];
    for (const section of navSections) {
        for (const item of section.items) {
            if (!FIXED_TAB_ROUTES.has(item.routeName)) {
                result.push({
                    name: item.name,
                    to: item.to,
...
const navSections = [
    {
        ...
        items: [
            ...
            { name: 'Formularios', to: '/admin/formularios', icon: 'forms', routeName: 'admin-forms' },
        ],
    },
    {
        label: 'Financiero',
    ...
    },
    {
        label: 'Marketing',
        items: [
            { name: 'Cola de Drops', to: '/admin/marketing/queue', icon: 'strategy', routeName: 'admin-marketing-queue', badge: 'NUEVO' },
        ],
    },
    {
        label: 'Planes',
...
<template>
  ...
  <div class="admin-shell min-h-screen bg-wc-bg text-wc-text">
          ...
          <ul class="space-y-0.5">
            <li v-for="item in section.items" :key="item.routeName">
              ...
              >
                ...
                <!-- Forms (clipboard with eye / preview icon) -->
                <svg v-else-if="item.icon === 'forms'" class="h-[18px] w-[18px] shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <span v-if="item.badge" class="ml-2 rounded-full bg-wc-accent px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-white">{{ item.badge }}</span>
              </RouterLink>
            </li>
          </ul>
        </div>
    ...
    <!-- Mobile Bottom Navigation — Fase A admin v2 -->
    <AdminBottomNav :drawer-items="drawerItems" />
