...
const REASONS = computed(() => [
    { value: 'spam',      label: t('client_social.report_reason_spam') },
    { value: 'offensive', label: t('client_social.report_reason_offensive') },
    { value: 'off_topic', label: t('client_social.report_reason_off_topic') },
    { value: 'other',     label: t('client_social.report_reason_other') },
]);
...
async function submit() {
    if (!reason.value) {
        toast.warn(t('client_social.report_reason_required'));
        return;
    }
    ...
    try {
        ...
        haptics.success();
        toast.success(t('client_social.report_success'));
        emit('reported', props.postId);
        close();
    } catch (err) {
        haptics.error();
        toast.apiError(err, t('client_social.report_failed'));
    } finally {
...
<template>
  <div class="relative">
    <button @click="menuOpen = !menuOpen" class="rounded-lg p-1.5 text-wc-text-tertiary hover:text-wc-text hover:bg-wc-bg-tertiary" :aria-label="t('client_social.report_menu_options')">
    ...
    <Transition enter-active-class="duration-150" enter-from-class="opacity-0 scale-95">
      <div v-if="menuOpen" class="absolute right-0 top-full mt-1 w-48 rounded-xl border border-wc-border bg-wc-bg-secondary shadow-xl z-20 py-1">
        <button @click="openModal" class="w-full text-left px-3 py-2 text-sm text-rose-500 hover:bg-rose-500/10 flex items-center gap-2">
          <span>🚩</span><span>{{ t('client_social.report_menu_report') }}</span>
        </button>
      </div>
    </Transition>
      ...
      <div v-if="modalOpen" class="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" @click.self="close">
        <div class="rounded-2xl bg-wc-bg-secondary border border-wc-border w-full max-w-md p-6">
          <header class="flex items-center justify-between mb-4">
            <h3 class="font-display text-xl text-wc-text">{{ t('client_social.report_modal_title') }}</h3>
          ...
          <p class="text-sm text-wc-text-secondary mb-3">{{ t('client_social.report_modal_question') }}</p>
          ...
          <textarea v-model="detail" rows="3" maxlength="500" :placeholder="t('client_social.report_detail_placeholder')" class="w-full rounded-lg border border-wc-border bg-wc-bg p-2 text-sm text-wc-text mb-4 resize-none"></textarea>
          <div class="flex gap-3">
            <button @click="close" class="flex-1 rounded-full border border-wc-border text-wc-text-secondary py-2">{{ t('client_social.report_cancel') }}</button>
            <button @click="submit" :disabled="!reason || submitting" class="flex-1 rounded-full bg-wc-accent text-white py-2 font-semibold disabled:opacity-50">
              {{ submitting ? t('client_social.report_submitting') : t('client_social.report_submit') }}
            </button>
          </div>
        </div>
      </div>
