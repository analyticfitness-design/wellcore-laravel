...
const xpDeltaText = computed(() => {
    ...
    return `${inLevel} / 200 al nivel ${nextLevel}`;
});

// Streak label "1 día consecutivo" / "X días consecutivos"
const streakUnitText = computed(() => {
    const d = props.data.streakDays || 0;
    return d === 1 ? 'día consecutivo' : 'días consecutivos';
});
...
<template>
  <div class="stats-grid section" :style="{ animationDelay: '220ms' }">
    ...
    <div class="stat-card purple">
      ...
      <div class="stat-label">Nivel {{ data.level || 1 }}</div>
      <div class="stat-value tight tnum">{{ (data.xpTotal || 0).toLocaleString() }}<span class="unit">XP</span></div>
    ...
    <div class="stat-card amber">
      ...
      <div class="stat-sub">días entrenados</div>
    </div>
  </div>
</template>
