    ...
    <div v-else class="mx-auto max-w-4xl space-y-8">
      ...
      >
        <div v-if="showSuccess" class="flex items-center justify-between rounded-xl border border-green-500/30 bg-green-500/10 px-4 py-3">
          ...
          <button @click="dismissSuccess" class="text-green-400/60 transition-colors hover:text-green-400">
      ...
      <div class="grid grid-cols-3 gap-3">
        <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-4 text-center">
          <p class="font-data text-2xl font-bold text-wc-accent">{{ stats.total }}</p>
          <p class="mt-1 text-[10px] font-medium uppercase tracking-wider text-wc-text-tertiary">Total referidos</p>
        </div>
        <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-4 text-center">
          <p class="font-data text-2xl font-bold text-blue-400">{{ stats.registered }}</p>
          <p class="mt-1 text-[10px] font-medium uppercase tracking-wider text-wc-text-tertiary">Registrados</p>
        </div>
        <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-4 text-center">
          <p class="font-data text-2xl font-bold text-emerald-500">{{ stats.active }}</p>
          <p class="mt-1 text-[10px] font-medium uppercase tracking-wider text-wc-text-tertiary">Activos</p>
        </div>
      </div>
      ...
      <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-5">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          >
            <svg v-if="!copied" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0 0 13.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 0 1-.75.75H9a.75.75 0 0 1-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 0 1-2.25 2.25H6.75A2.25 2.25 0 0 1 4.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 0 1 1.927-.184" />
            </svg>
            <svg v-else class="h-4 w-4 text-green-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
            </svg>
      ...
      <!-- Invite Form -->
      <div class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-5">
        <h3 class="mb-3 font-semibold text-wc-text">Invitar por correo</h3>
        <form @submit.prevent="sendInvite" class="flex items-start gap-3">
          <div class="flex-1">
            <input
              ...
              class="w-full rounded-lg border border-wc-border bg-wc-bg px-4 py-2.5 text-sm text-wc-text placeholder-wc-text-tertiary transition-colors focus:border-wc-accent/50 focus:outline-none focus:ring-2 focus:ring-wc-accent/20"
            />
            <p v-if="inviteError" class="mt-1 text-xs text-red-400">{{ inviteError }}</p>
          </div>
          <button
            type="submit"
            :disabled="inviteSending || !inviteEmail.trim()"
            class="rounded-lg bg-wc-accent px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-wc-accent/90 disabled:opacity-50"
          >
            <span v-if="inviteSending">Enviando...</span>
      ...
      <div v-if="history.length > 0" class="rounded-xl border border-wc-border bg-wc-bg-tertiary">
        <div class="border-b border-wc-border px-5 py-3">
          <h3 class="text-xs font-semibold uppercase tracking-wider text-wc-text-tertiary">Historial de referidos</h3>
        </div>
        <div class="divide-y divide-wc-border/50">
          ...
          >
            <div class="flex items-center gap-3">
              <div class="flex h-9 w-9 items-center justify-center rounded-full bg-wc-bg-secondary">
                <span class="text-xs font-bold text-wc-text-tertiary">
                  {{ (ref_item.name || ref_item.email || 'U').charAt(0).toUpperCase() }}
                </span>
              </div>
              <div>
                <p class="text-sm font-medium text-wc-text">{{ ref_item.name || ref_item.email }}</p>
                <p class="text-xs text-wc-text-tertiary">{{ formatDate(ref_item.created_at) }}</p>
              </div>
            </div>
            <span
      ...
      <div v-else class="rounded-2xl border border-dashed border-wc-border bg-wc-bg-tertiary/50 p-12 text-center">
        ...
        <h3 class="mt-4 font-display text-xl text-wc-text">SIN REFERIDOS AUN</h3>
        <p class="mx-auto mt-2 max-w-xs text-sm text-wc-text-secondary">Comparte tu link y comienza a ganar recompensas.</p>
      </div>
    </div>
  </ClientLayout>
</template>
