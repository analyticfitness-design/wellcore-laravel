...
const voice = useVoiceTranscription({ lang: locale.value === 'en' ? 'en-US' : 'es-CO' });
...
function onFileSelected(e, meal) {
    ...
    if (!allowed.includes(file.type)) {
        toast.show(t('client_progress.food_err_format'), 'error');
        e.target.value = '';
        return;
    }
    // Validación tamaño (10 MB máximo)
    if (file.size > 10 * 1024 * 1024) {
        toast.show(t('client_progress.food_err_size'), 'error');
        e.target.value = '';
        return;
    }
...
async function confirmUpload(meal) {
    ...
    try {
        ...
        uploadErrors[meal.index] = err.response?.data?.message || t('client_progress.food_err_upload_generic');
    }
}
...
async function removePhoto(meal) {
    ...
    if (!confirm(t('client_progress.food_confirm_delete'))) return;
    deleteError.value = '';
    try {
        await food.deletePhoto(meal.photo.id);
    } catch (err) {
        deleteError.value = t('client_progress.food_err_delete');
...
function onPostNoteInput(photo, value) {
    ...
    _saveNoteTimer = setTimeout(async () => {
        ...
        try {
            ...
            noteError.value = t('client_progress.food_err_save_note');
...
async function startDictating(target) {
    ...
    if (!voice.supported.value) {
        alert(voice.error.value || t('client_progress.food_no_voice_alert'));
        return;
    }
    ...
    try {
        ...
        noteError.value = t('client_progress.food_err_dictation');
        setTimeout(() => { noteError.value = ''; }, 4000);
    } finally {
  ...
  <ClientLayout>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="flex items-end justify-between">
        <div>
          <h1 class="font-display text-3xl tracking-wide text-wc-text">{{ t('client_progress.food_title').toUpperCase() }}</h1>
          <p class="mt-1 text-sm text-wc-text-secondary">{{ t('client_progress.food_subtitle') }}</p>
        </div>
        <div class="flex flex-col items-end gap-1">
          <span v-if="food.streakDays.value > 0"
            ...
            🔥 {{ t('client_progress.food_streak_days', { n: food.streakDays.value }) }}
          </span>
          <span class="rounded-full bg-amber-500/10 px-2 py-0.5 text-xs font-bold text-amber-400">
            {{ t('client_progress.food_xp_today', { n: food.xpToday.value }) }}
          </span>
        </div>
      </div>
      ...
      <div v-if="!food.hasNutritionPlan.value"
        ...
        <span class="text-wc-accent">ℹ</span> {{ t('client_progress.food_no_plan_notice') }}
      </div>
      ...
      <div class="rounded-xl border border-wc-border bg-wc-bg-secondary p-5">
        <div class="flex items-center justify-between">
          <p class="text-sm font-medium text-wc-text-secondary">
            {{ t('client_progress.food_progress_label', { done: food.completedToday.value, total: food.totalToday.value }) }}
          </p>
        ...
        <p v-if="food.bonusEarnedToday.value" class="mt-2 text-xs font-semibold text-amber-400">
          🎉 {{ t('client_progress.food_bonus_complete') }}
        </p>
      </div>
        ...
        >
          <div class="flex items-center gap-3 p-4">
            ...
            <div class="min-w-0 flex-1">
              <p class="truncate font-display text-sm tracking-wide text-wc-text">{{ (meal.nombre || t('client_progress.food_meal_generic')).toUpperCase() }}</p>
              <p v-if="meal.calorias > 0" class="text-[11px] text-wc-text-tertiary">
                {{ meal.calorias }} {{ t('client_progress.food_kcal_unit') }}
              </p>
            </div>
            <span v-if="meal.photo?.xp_awarded"
                  class="rounded-full bg-amber-500/10 px-2 py-0.5 text-xs font-bold text-amber-400">{{ t('client_progress.food_xp_per_meal') }}</span>
          </div>

          <!-- Foto ya subida -->
          <div v-if="meal.photo" class="relative group">
            <img :src="meal.photo.photo_url" :alt="t('client_progress.food_alt_meal_photo', { meal: meal.nombre })"
            ...
            <div v-if="meal.photo.coach_seen"
              ...
              {{ meal.photo.coach_reaction === 'bien' ? '✅ ' + t('client_progress.food_reaction_good') : meal.photo.coach_reaction === 'mejorar' ? '⚠️ ' + t('client_progress.food_reaction_improve') : '👁 ' + t('client_progress.food_reaction_seen') }}
            </div>
          </div>
          ...
          <div v-if="meal.photo && !meal.photo.coach_seen"
            ...
            <div class="mb-1 flex items-center justify-between">
              <label class="text-[11px] uppercase tracking-wider text-wc-text-tertiary">{{ t('client_progress.food_your_description') }}</label>
              <span v-if="savingNote[meal.photo.id]" class="text-[10px] text-wc-text-tertiary">{{ t('client_progress.food_saving') }}</span>
            </div>
            <div class="relative">
              <textarea
                ...
                rows="2"
                :placeholder="t('client_progress.food_describe_placeholder')"
              ...
              <button v-if="voice.supported.value"
                      ...
                      type="button"
                      :title="recordingFor === `post-${meal.photo.id}` ? t('client_progress.food_dictate_stop') : t('client_progress.food_dictate_start')"
          ...
          <div v-else-if="meal.photo?.client_note"
               class="border-t border-wc-border bg-wc-bg px-4 py-3">
            <p class="text-[11px] uppercase tracking-wider text-wc-text-tertiary">{{ t('client_progress.food_your_description') }}</p>
          ...
          <div v-if="!meal.photo && pendingFile[meal.index]" class="border-t border-wc-border">
            <div class="relative">
              <img :src="pendingPreview[meal.index]" :alt="t('client_progress.food_alt_preview', { meal: meal.nombre })"
                   class="w-full max-h-72 object-cover">
              <span class="absolute left-3 top-3 rounded-full bg-amber-500/90 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white">
                {{ t('client_progress.food_pre_pending_pill') }}
              </span>
            </div>
            <div class="space-y-3 p-4">
              <div>
                <div class="mb-1 flex items-center justify-between">
                  <label class="text-[11px] uppercase tracking-wider text-wc-text-tertiary">{{ t('client_progress.food_what_you_ate') }}</label>
                  <span v-if="recordingFor === `pre-${meal.index}`" class="flex items-center gap-1 text-[10px] font-bold text-red-400">
                    <span class="h-1.5 w-1.5 animate-pulse rounded-full bg-red-500"></span>
                    {{ t('client_progress.food_recording') }}
                  </span>
                </div>
                <div class="relative">
                  <textarea
                    ...
                    rows="2"
                    :placeholder="t('client_progress.food_pre_placeholder')"
                    class="w-full resize-none rounded-lg border border-wc-border bg-wc-bg-secondary p-2 pr-10 text-sm text-wc-text"
                  ></textarea>
                  <button v-if="voice.supported.value"
                          @click="recordingFor === `pre-${meal.index}` ? stopDictating() : startDictating({ kind: 'pre', index: meal.index })"
                          type="button"
                          :title="recordingFor === `pre-${meal.index}` ? t('client_progress.food_dictate_stop') : t('client_progress.food_dictate_start')"
                          class="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full transition"
                ...
                <p v-if="!voice.supported.value" class="mt-1 text-[10px] text-wc-text-tertiary">
                  {{ t('client_progress.food_no_voice_hint') }}
                </p>
              </div>
              ...
              <div class="flex gap-2">
                ...
                <button @click="confirmUpload(meal)"
                  ...
                  {{ food.uploadingIndex.value === meal.index ? t('client_progress.food_uploading') : t('client_progress.food_confirm_photo') }}
                </button>
              </div>
          ...
          <button v-else-if="!meal.photo"
            ...
            {{ food.uploadingIndex.value === meal.index ? t('client_progress.food_uploading') : t('client_progress.food_upload_photo') }}
          </button>

          <div v-if="meal.photo?.coach_note"
               class="border-t border-wc-border bg-wc-bg-tertiary px-4 py-3">
            <p class="text-xs uppercase tracking-wider text-wc-text-tertiary">{{ t('client_progress.food_coach_note_label') }}</p>
