<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      <select v-model="mealFilter" class="rounded-lg border border-wc-border bg-wc-bg-tertiary px-3 py-2.5 text-sm text-wc-text focus:border-wc-accent focus:outline-none">
      ...
      <select v-model="timeFilter" class="rounded-lg border border-wc-border bg-wc-bg-tertiary px-3 py-2.5 text-sm text-wc-text focus:border-wc-accent focus:outline-none">
    ...
    <div class="flex flex-wrap gap-2">
      <button
        ...
        :class="goalFilter === 'all' ? 'bg-wc-accent text-white' : 'bg-wc-bg-tertiary text-wc-text-secondary hover:text-wc-text'"
        class="rounded-lg border border-wc-border px-4 py-2 text-sm font-medium transition-colors"
      >
        Todas <span class="ml-1 text-xs opacity-70">({{ RECIPES.length }})</span>
      </button>
      <button
        v-for="g in GOALS"
        :key="g.id"
        @click="goalFilter = g.id"
        :class="goalFilter === g.id ? 'bg-wc-accent text-white' : 'bg-wc-bg-tertiary text-wc-text-secondary hover:text-wc-text'"
        class="rounded-lg border border-wc-border px-4 py-2 text-sm font-medium transition-colors"
      >
        {{ g.icon }} {{ g.label }}
      </button>
    ...
    <p class="text-xs text-wc-text-tertiary">
      ...
      <span v-if="goalFilter !== 'all'"> para <span class="text-wc-accent">{{ GOALS.find(g => g.id === goalFilter)?.label }}</span></span>
    </p>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div
        ...
        class="group cursor-pointer rounded-xl border border-wc-border bg-wc-bg-tertiary transition-all hover:border-wc-accent/40"
      >
        <!-- Thumbnail -->
        <div class="relative aspect-[4/3] overflow-hidden rounded-t-xl bg-wc-bg-secondary">
          ...
          <span class="absolute right-2 top-2 rounded-full bg-wc-accent/80 px-2 py-0.5 text-[10px] font-semibold text-white">{{ r.meal }}</span>
        </div>

        <!-- Info -->
        <div class="p-4">
          <h3 class="text-sm font-semibold text-wc-text">{{ r.name }}</h3>
          <p class="mt-1 line-clamp-2 text-xs text-wc-text-tertiary">{{ r.description }}</p>

          <!-- Macro bar -->
          <div class="mt-3 grid grid-cols-4 gap-1 text-center">
            <div>
              <p class="font-data text-sm font-bold text-wc-text">{{ r.macros.cal }}</p>
              <p class="text-[9px] text-wc-text-tertiary">kcal</p>
            </div>
            <div>
              <p class="font-data text-sm font-bold text-blue-400">{{ r.macros.protein }}g</p>
              <p class="text-[9px] text-wc-text-tertiary">Proteina</p>
            </div>
            <div>
              <p class="font-data text-sm font-bold text-yellow-400">{{ r.macros.carbs }}g</p>
              <p class="text-[9px] text-wc-text-tertiary">Carbos</p>
            </div>
            <div>
              <p class="font-data text-sm font-bold text-orange-400">{{ r.macros.fat }}g</p>
    ...
    <!-- Empty state -->
    <div v-if="filteredRecipes.length === 0" class="rounded-xl border border-wc-border bg-wc-bg-tertiary p-12 text-center">
    ...
    <Transition name="fade">
      ...
      >
        <Transition name="scale">
          <div v-if="selectedRecipe" class="w-full max-w-2xl rounded-2xl border border-wc-border bg-wc-bg-secondary">
            <!-- Modal header -->
            <div class="flex items-center justify-between border-b border-wc-border px-6 py-4">
              <div class="flex items-center gap-3">
                <span class="text-2xl">{{ selectedRecipe.emoji }}</span>
                <h2 class="font-display text-xl tracking-wide text-wc-text">{{ selectedRecipe.name }}</h2>
              </div>
              <button @click="selectedRecipe = null" class="flex h-8 w-8 items-center justify-center rounded-lg text-wc-text-secondary hover:text-wc-text">
            ...
            <div class="max-h-[70vh] overflow-y-auto px-6 py-5 space-y-5">
              ...
              <div class="grid grid-cols-4 gap-3">
                <div class="rounded-lg border border-wc-border bg-wc-bg p-3 text-center">
                  <p class="font-data text-xl font-bold text-wc-text">{{ selectedRecipe.macros.cal }}</p>
                ...
                <div class="rounded-lg border border-blue-500/20 bg-blue-500/5 p-3 text-center">
                  <p class="font-data text-xl font-bold text-blue-400">{{ selectedRecipe.macros.protein }}g</p>
                ...
                <div class="rounded-lg border border-yellow-500/20 bg-yellow-500/5 p-3 text-center">
                  <p class="font-data text-xl font-bold text-yellow-400">{{ selectedRecipe.macros.carbs }}g</p>
                ...
                <div class="rounded-lg border border-orange-500/20 bg-orange-500/5 p-3 text-center">
                  <p class="font-data text-xl font-bold text-orange-400">{{ selectedRecipe.macros.fat }}g</p>
              ...
              <!-- Meta tags -->
              <div class="flex flex-wrap gap-2">
                <span class="rounded-lg bg-wc-accent/10 px-3 py-1 text-xs font-semibold text-wc-accent">{{ selectedRecipe.meal }}</span>
                <span class="rounded-lg bg-wc-bg-tertiary px-3 py-1 text-xs font-medium text-wc-text-secondary">{{ selectedRecipe.prepTime }} min</span>
                <span class="rounded-lg bg-wc-bg-tertiary px-3 py-1 text-xs font-medium text-wc-text-secondary">{{ selectedRecipe.servings }}</span>
                <span
                  v-for="tag in selectedRecipe.tags"
                  :key="tag"
                  class="rounded-lg bg-wc-bg-tertiary px-3 py-1 text-xs font-medium text-wc-text-tertiary"
                >{{ tag }}</span>
              </div>
              ...
              <div>
                ...
                <ul class="mt-2 space-y-1.5">
                  <li v-for="(ing, i) in selectedRecipe.ingredients" :key="i" class="flex items-center gap-2 text-sm text-wc-text-secondary">
              ...
              <div>
                ...
                <ol class="mt-2 space-y-3">
                  <li v-for="(step, i) in selectedRecipe.steps" :key="i" class="flex gap-3 text-sm text-wc-text-secondary">
                    <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-wc-accent/10 text-[10px] font-bold text-wc-accent">{{ i + 1 }}</span>
                    {{ step }}
                  </li>
                </ol>
              </div>
              ...
              <!-- Coach tip -->
              <div v-if="selectedRecipe.coachTip" class="rounded-lg border border-wc-accent/20 bg-wc-accent/5 p-4">
