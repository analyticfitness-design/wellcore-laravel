<template>
  ...
  <div class="space-y-8 px-4 sm:px-0 sm:space-y-10">
    ...
    <div class="relative overflow-hidden rounded-2xl border border-wc-border bg-wc-bg-secondary px-6 py-8 sm:px-10 sm:py-10">
      ...
      <div class="relative">
        ...
        <span class="mb-3 inline-flex items-center gap-1.5 rounded-full border border-wc-accent/30 bg-wc-accent/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-wc-accent">
          ...
          {{ t('client_social.mindfulness_eyebrow') }}
        </span>

        <h1 class="font-display text-4xl tracking-wide text-wc-text sm:text-5xl">
          {{ t('client_social.mindfulness_title_a') }} <span class="text-wc-accent">{{ t('client_social.mindfulness_title_b') }}</span>
        </h1>
        <p class="mt-2 max-w-lg text-sm text-wc-text-secondary">
          {{ t('client_social.mindfulness_subtitle') }}
        </p>
      </div>
    </div>

    <!-- ═══ SECTION 1: Sesiones Guiadas ══════════════════════════════════════ -->
    <section :aria-label="t('client_social.mindfulness_sessions_aria')">
      <h2 class="mb-5 font-display text-xl tracking-wide text-wc-text">{{ t('client_social.mindfulness_sessions_heading') }}</h2>
      ...
      <Transition name="fade" mode="out-in">
        ...
        >
          ...
          <div class="relative mb-6 flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <span class="inline-flex items-center gap-1.5 rounded-full bg-wc-accent/15 px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-wc-accent">
                ...
                {{ t('client_social.mindfulness_session_active_badge') }}
              </span>
            ...
            >
              {{ t('client_social.mindfulness_session_end') }}
            </button>
          </div>
          ...
          <div class="relative flex flex-col items-center gap-6">
            <div class="relative flex h-44 w-44 items-center justify-center sm:h-52 sm:w-52">
              ...
              <div class="relative text-center" aria-live="polite" aria-atomic="true">
                ...
                <p class="mt-1 wc-caption">{{ t('client_social.mindfulness_session_remaining') }}</p>
              </div>
            </div>
            ...
            <Transition name="fade">
              <div v-if="sessionRemaining === 0 && !sessionRunning" class="rounded-xl border border-green-500/30 bg-green-500/10 px-6 py-3 text-center">
                <p class="text-sm font-semibold text-green-400">{{ t('client_social.mindfulness_session_completed_title') }}</p>
                <p class="mt-1 text-xs text-wc-text-secondary">{{ t('client_social.mindfulness_session_completed_desc') }}</p>
              </div>
            </Transition>
            ...
            <div class="flex items-center gap-3">
              ...
              >
                {{ t('client_social.mindfulness_session_pause') }}
              </button>
              <button
              ...
              >
                {{ t('client_social.mindfulness_session_resume') }}
              </button>
            </div>
          </div>
          ...
          <div class="relative mt-6" role="progressbar" :aria-valuenow="Math.round(sessionProgress)" aria-valuemin="0" aria-valuemax="100">
            ...
            <div class="mt-1.5 flex justify-between text-xs text-wc-text-tertiary">
              <span>{{ t('client_social.mindfulness_session_progress_start') }}</span>
              <span>{{ Math.round(sessionProgress) }}%</span>
              <span>{{ t('client_social.mindfulness_session_progress_end') }}</span>
            </div>
          </div>
        </div>
        ...
        <div v-else key="cards" class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <button
            ...
            :aria-label="t('client_social.mindfulness_session_aria_start', { title: session.title })"
    ...
    <section :aria-label="t('client_social.mindfulness_breathing_aria')">
      <h2 class="mb-5 font-display text-xl tracking-wide text-wc-text">{{ t('client_social.mindfulness_breathing_heading') }}</h2>

      <!-- Technique Tabs — horizontal scroll on mobile -->
      <div class="overflow-x-auto pb-1" role="tablist" :aria-label="t('client_social.mindfulness_breathing_tablist_aria')">
      ...
      <div class="mx-auto mt-6 max-w-md">
        ...
        >
          ...
          >
            ...
            <span v-if="running || paused" class="text-[10px] tracking-wider" style="color: rgba(255,255,255,0.35)">
              {{ t('client_social.mindfulness_ring_cycle', { current: currentCycle, total: totalCycles }) }}
            </span>
            <span v-if="!running && !paused" class="text-[11px] font-semibold uppercase tracking-widest" style="color: rgba(255,255,255,0.28)">
              {{ t('client_social.mindfulness_ring_press_start') }}
            </span>
          </div>
        </div>
        ...
        <div v-if="!running && !paused" class="mt-6 space-y-4 rounded-xl border border-wc-border bg-wc-bg-tertiary p-4 sm:p-6">
          ...
          <div>
            <label class="block text-xs font-medium text-wc-text-secondary">{{ t('client_social.mindfulness_config_cycles') }}</label>
            <input
              type="number"
              v-model.number="totalCycles"
              min="1" max="20"
              :aria-label="t('client_social.mindfulness_config_cycles_aria')"
            ...
            <p class="mt-1 text-xs text-wc-text-secondary">
              {{ t('client_social.mindfulness_config_total_duration', { time: formatBreathTime(totalCycles * currentTechnique.cycleDuration) }) }}
            </p>
          </div>
          <label class="flex cursor-pointer items-center gap-3">
            <div class="relative">
              <input type="checkbox" v-model="soundEnabled" class="sr-only peer" :aria-label="t('client_social.mindfulness_config_sound_aria')" />
            ...
            <span class="text-xs text-wc-text-secondary">{{ t('client_social.mindfulness_config_sound') }}</span>
          </label>
        </div>
        ...
        <div class="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-center sm:gap-4">
          ...
          >
            {{ t('client_social.mindfulness_start') }}
          </button>
          <button
            v-if="running"
          ...
          >
            {{ t('client_social.mindfulness_pause') }}
          </button>
          <button
            v-if="paused"
            ...
            class="w-full rounded-xl bg-wc-accent px-10 py-3 text-sm font-bold uppercase tracking-wider text-white transition-colors hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-wc-accent sm:w-auto"
          >
            {{ t('client_social.mindfulness_resume') }}
          </button>
          <button
            v-if="running || paused"
            @click="stopBreathing"
          ...
          >
            {{ t('client_social.mindfulness_stop') }}
          </button>
        </div>
        ...
        <Transition name="fade">
          <div v-if="completed" class="mt-6 rounded-xl border border-green-500/30 bg-green-500/10 p-6 text-center" role="status">
            ...
            <p class="mt-3 text-sm font-semibold text-green-400">{{ t('client_social.mindfulness_completed_title') }}</p>
            <p class="mt-1 text-xs text-wc-text-secondary">
              {{ t('client_social.mindfulness_completed_meta', { label: currentTechnique.label, cycles: totalCycles, time: formatBreathTime(totalCycles * currentTechnique.cycleDuration) }) }}
            </p>
            <button
            ...
            >
              {{ t('client_social.mindfulness_close') }}
            </button>
          </div>
        </Transition>
        ...
        <div class="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div class="rounded-lg border border-wc-border bg-wc-bg-tertiary p-4">
            <div class="flex items-center gap-2">
              ...
              <p class="text-xs font-semibold text-wc-accent">{{ t('client_social.mindfulness_info_478_heading') }}</p>
            </div>
            <p class="mt-2 text-xs leading-relaxed text-wc-text-secondary">{{ t('client_social.mindfulness_info_478_desc') }}</p>
          </div>
          <div class="rounded-lg border border-wc-border bg-wc-bg-tertiary p-4">
            <div class="flex items-center gap-2">
              <span class="inline-block h-2.5 w-2.5 rounded-full bg-blue-500"></span>
              <p class="text-xs font-semibold text-blue-400">{{ t('client_social.mindfulness_info_box_heading') }}</p>
            </div>
            <p class="mt-2 text-xs leading-relaxed text-wc-text-secondary">{{ t('client_social.mindfulness_info_box_desc') }}</p>
          </div>
          <div class="rounded-lg border border-wc-border bg-wc-bg-tertiary p-4">
            <div class="flex items-center gap-2">
              <span class="inline-block h-2.5 w-2.5 rounded-full bg-purple-500"></span>
              <p class="text-xs font-semibold text-purple-400">{{ t('client_social.mindfulness_info_coherent_heading') }}</p>
            </div>
            <p class="mt-2 text-xs leading-relaxed text-wc-text-secondary">{{ t('client_social.mindfulness_info_coherent_desc') }}</p>
          </div>
        </div>
...
// Sessions / techniques use computed so labels react to locale change.
const SESSIONS = computed(() => [
  { id: 'breathing',     title: t('client_social.mindfulness_session_4_7_8_title'),         description: t('client_social.mindfulness_session_4_7_8_desc'),         duration: '5 min',  emoji: '🌬️', benefit: t('client_social.mindfulness_session_4_7_8_benefit'),         seconds: 300 },
  { id: 'meditation',    title: t('client_social.mindfulness_session_meditation_title'),    description: t('client_social.mindfulness_session_meditation_desc'),    duration: '10 min', emoji: '🧘', benefit: t('client_social.mindfulness_session_meditation_benefit'),    seconds: 600 },
  { id: 'body-scan',     title: t('client_social.mindfulness_session_body_scan_title'),     description: t('client_social.mindfulness_session_body_scan_desc'),     duration: '15 min', emoji: '🔍', benefit: t('client_social.mindfulness_session_body_scan_benefit'),     seconds: 900 },
  { id: 'visualization', title: t('client_social.mindfulness_session_visualization_title'), description: t('client_social.mindfulness_session_visualization_desc'), duration: '8 min',  emoji: '🏆', benefit: t('client_social.mindfulness_session_visualization_benefit'), seconds: 480 },
]);

const TECHNIQUES = computed(() => ({
  '478': {
    id: '478', label: t('client_social.mindfulness_tech_478_label'),
    description: t('client_social.mindfulness_tech_478_desc'),
    cycleDuration: 19,
    phases: [
      { type: 'inhala', label: t('client_social.mindfulness_phase_inhale_short'), seconds: 4 },
      { type: 'manten', label: t('client_social.mindfulness_phase_hold_short'),   seconds: 7 },
      { type: 'exhala', label: t('client_social.mindfulness_phase_exhale_short'), seconds: 8 },
    ],
  },
  'box': {
    id: 'box', label: t('client_social.mindfulness_tech_box_label'),
    description: t('client_social.mindfulness_tech_box_desc'),
    cycleDuration: 16,
    phases: [
      { type: 'inhala', label: t('client_social.mindfulness_phase_inhale_short'), seconds: 4 },
      { type: 'manten', label: t('client_social.mindfulness_phase_hold_short'),   seconds: 4 },
      { type: 'exhala', label: t('client_social.mindfulness_phase_exhale_short'), seconds: 4 },
      { type: 'manten', label: t('client_social.mindfulness_phase_hold_short'),   seconds: 4 },
    ],
  },
  'coherent': {
    id: 'coherent', label: t('client_social.mindfulness_tech_coherent_label'),
    description: t('client_social.mindfulness_tech_coherent_desc'),
    cycleDuration: 11,
    phases: [
      { type: 'inhala', label: t('client_social.mindfulness_phase_inhale_short'), seconds: 5.5 },
      { type: 'exhala', label: t('client_social.mindfulness_phase_exhale_short'), seconds: 5.5 },
    ],
  },
}));

const TECHNIQUE_LIST = computed(() => [
  { id: '478',      label: t('client_social.mindfulness_tech_478_tab') },
  { id: 'box',      label: t('client_social.mindfulness_tech_box_tab') },
  { id: 'coherent', label: t('client_social.mindfulness_tech_coherent_tab') },
]);
...
function startSession(type) {
  const session = SESSIONS.value.find(s => s.id === type);
  if (!session) return;
...
const currentTechnique = computed(() => TECHNIQUES.value[technique.value]);

const phaseLabel = computed(() => {
  const labels = {
    inhala: t('client_social.mindfulness_phase_inhale'),
    manten: t('client_social.mindfulness_phase_hold'),
    exhala: t('client_social.mindfulness_phase_exhale'),
  };
