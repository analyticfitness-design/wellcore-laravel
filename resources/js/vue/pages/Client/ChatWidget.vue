...
const messages = ref([]);
const coachName = ref(t('client_social.chat_role_coach'));
...
async function fetchChat() {
    ...
    try {
        ...
        coachName.value = d.coach_name || t('client_social.chat_role_coach');
        ...
        error.value = err.response?.data?.message || t('client_social.chat_load_error');
    } finally {
        loading.value = false;
    }
}
...
async function sendMessage() {
    ...
    try {
        ...
        if (err.response?.status === 422) {
            ...
            toast.apiError(err, t('client_social.chat_send_error'));
        }
    } finally {
        sending.value = false;
    }
...
<template>
  <ClientLayout>
    ...
    <div v-else class="space-y-6">
      <!-- Header -->
      <div>
        <h1 class="font-display text-3xl tracking-wide text-wc-text">{{ t('client_social.chat_title') }}</h1>
        <p class="mt-1 text-sm text-wc-text-secondary">{{ t('client_social.chat_subtitle') }}</p>
      </div>
      ...
      <div class="overflow-hidden rounded-xl border border-wc-border bg-wc-bg-tertiary">
        ...
        <div class="flex items-center gap-3 border-b border-wc-border px-5 py-3">
          ...
          <div class="flex-1">
            ...
            <div v-if="hasCoach" class="flex items-center gap-1.5">
              ...
              <span class="text-sm text-wc-text-tertiary">{{ t('client_social.chat_role_coach') }}</span>
            </div>
            <span v-else class="text-sm text-wc-text-tertiary/70">{{ t('client_social.chat_no_coach_assigned') }}</span>
          </div>
          <!-- Realtime indicator -->
          <div v-if="echoChannel" class="flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5">
            <span class="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
            <span class="text-[10px] font-medium text-emerald-500">{{ t('client_social.chat_live_indicator') }}</span>
          </div>
        </div>
        ...
        <div v-if="!hasCoach" class="flex flex-col items-center justify-center px-6 py-16 text-center">
          ...
          <h3 class="mt-4 font-display text-xl text-wc-text">{{ t('client_social.chat_no_coach_title') }}</h3>
          <p class="mt-2 max-w-xs text-sm text-wc-text-secondary">
            {{ t('client_social.chat_no_coach_desc') }}
          </p>
        </div>
        ...
        <template v-else>
          ...
          >
            ...
            <div v-if="messages.length === 0" class="flex h-full flex-col items-center justify-center text-center">
              ...
              <h3 class="mt-4 font-display text-xl text-wc-text">{{ t('client_social.chat_empty_title') }}</h3>
              <p class="mt-2 max-w-xs text-sm text-wc-text-secondary">{{ t('client_social.chat_empty_desc') }}</p>
            </div>
            ...
            <form @submit.prevent="sendMessage" class="flex items-end gap-3">
              <div class="flex-1">
                <textarea
                  ...
                  rows="1"
                  maxlength="2000"
                  :placeholder="t('client_social.chat_placeholder')"
                  :aria-label="t('client_social.chat_aria_input')"
              ...
              <button
                ...
                :aria-label="t('client_social.chat_aria_send')"
              >
