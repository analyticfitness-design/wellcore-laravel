...
async function fetchSettings() {
    ...
    try {
        const response = await api.get('/api/v/client/settings');
  ...
  <ClientLayout>
    ...
    >
      <div
        ...
        class="fixed bottom-24 right-4 z-50 flex items-center gap-3 rounded-xl border border-green-500/30 bg-green-500/10 px-5 py-3 shadow-lg backdrop-blur-sm lg:bottom-6 lg:right-6"
      >
    ...
    <div class="mb-6 flex flex-wrap gap-2">
      <button
        @click="tab = 'perfil'"
        :class="tab === 'perfil' ? 'bg-wc-accent text-white' : 'bg-wc-bg-tertiary text-wc-text-secondary hover:text-wc-text'"
      ...
      <button
        ...
        :class="tab === 'notificaciones' ? 'bg-wc-accent text-white' : 'bg-wc-bg-tertiary text-wc-text-secondary hover:text-wc-text'"
        class="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors"
      >
        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
      ...
      <button
        ...
        :class="tab === 'apariencia' ? 'bg-wc-accent text-white' : 'bg-wc-bg-tertiary text-wc-text-secondary hover:text-wc-text'"
        class="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors"
      >
        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
      ...
      <button
        @click="tab = 'seguridad'"
        :class="tab === 'seguridad' ? 'bg-wc-accent text-white' : 'bg-wc-bg-tertiary text-wc-text-secondary hover:text-wc-text'"
        class="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors"
      >
        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <div v-show="tab === 'perfil'">
      <!-- Loading -->
      <div v-if="loadingProfile" class="max-w-xl">
        <div class="h-80 animate-pulse rounded-xl border border-wc-border bg-wc-bg-tertiary"></div>
      </div>
      ...
      <div v-else class="max-w-xl rounded-xl border border-wc-border bg-wc-bg-tertiary p-5">
        ...
        <form @submit.prevent="updateProfile" class="space-y-4">
          ...
          <div class="pt-2">
            ...
            >
              <svg v-if="savingProfile" class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-center justify-between rounded-lg border border-wc-border bg-wc-bg p-4">
              ...
              <button
                ...
                role="switch"
                :aria-checked="notifications.checkin"
              >
                <span
            ...
            <div class="flex items-center justify-between rounded-lg border border-wc-border bg-wc-bg p-4">
              ...
              <button
                ...
                class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-wc-accent focus:ring-offset-2 focus:ring-offset-wc-bg"
                role="switch"
                :aria-checked="notifications.coach"
              >
                <span
            ...
            <div class="flex items-center justify-between rounded-lg border border-wc-border bg-wc-bg p-4">
              ...
              <button
                ...
                class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-wc-accent focus:ring-offset-2 focus:ring-offset-wc-bg"
                role="switch"
                :aria-checked="notifications.achievements"
              >
                <span
    ...
    <div v-show="tab === 'seguridad'">
      <div class="max-w-xl rounded-xl border border-wc-border bg-wc-bg-tertiary p-5">
        ...
        <!-- Password error -->
        <div v-if="passwordError" class="mb-4 rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-400">
          {{ passwordError }}
        </div>
        ...
        <form @submit.prevent="changePassword" class="space-y-4">
          ...
          <div class="pt-2">
            <button
              ...
              class="flex items-center gap-2 rounded-lg bg-wc-accent px-6 py-2.5 text-sm font-semibold text-white transition-all hover:opacity-90 active:scale-[0.98] disabled:opacity-60"
            >
              <svg v-if="savingPassword" class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
