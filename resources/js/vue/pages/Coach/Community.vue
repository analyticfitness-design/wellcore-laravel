...
const TABS = computed(() => [
    { key: 'latido',         label: t('coach_inbox.community_tab_pulse'),   component: CoachLatidoTab },
    { key: 'posts',          label: t('coach_inbox.community_tab_posts'),   component: CoachPostsTab },
    { key: 'conversaciones', label: t('coach_inbox.community_tab_threads'), component: CoachConversacionesTab },
    { key: 'pulsos',         label: t('coach_inbox.community_tab_stories'), component: CoachPulsosTab },
    { key: 'logros',         label: t('coach_inbox.community_tab_wins'),    component: CoachLogrosTab },
]);
...
const activeTab = ref(TABS.value.find(tab => tab.key === route.hash.slice(1))?.key || 'latido');
const activeComponent = computed(() => TABS.value.find(tab => tab.key === activeTab.value)?.component);

const triggerPostsRefresh = ref(0);
let coachChannel = null;

function changeTab(key) {
    previousTabIndex.value = TABS.value.findIndex(tab => tab.key === activeTab.value);
    activeTab.value = key;
    router.replace({ hash: `#${key}` });
}

const transitionDirection = computed(() => {
    const newIdx = TABS.value.findIndex(tab => tab.key === activeTab.value);
...
function quickMessageHandler(e) {
    ...
    const days = client?.days_inactive || t('coach_inbox.community_quick_msg_days_default');
    announce.message.value = t('coach_inbox.community_quick_msg_template', { name: firstName, days });
...
watch(() => route.hash, (h) => {
    const key = h.slice(1);
    if (TABS.value.some(tab => tab.key === key) && key !== activeTab.value) {
        changeTab(key);
    }
});
</script>
  ...
  <CoachLayout>
    <div class="space-y-4">
      <header class="space-y-1">
        <h1 class="font-display text-3xl tracking-wide text-wc-text">{{ t('coach_inbox.community_title') }}</h1>
        <p class="text-sm text-wc-text-tertiary">
          {{ t('coach_inbox.community_subtitle') }}
        </p>
      </header>
      ...
      >
        ...
        <span class="text-sm font-semibold">{{ t('coach_inbox.community_message_team') }}</span>
      </button>
