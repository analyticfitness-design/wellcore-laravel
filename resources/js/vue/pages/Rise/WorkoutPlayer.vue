...
async function toggleSet(exIndex, setIndex) {
  ...
  if (set.completed && workoutStarted.value) {
    ...
    try {
      const response = await api.post('/api/v/rise/workout/complete-set', {
...
async function completeCardioSet(exIndex, setIndex, duration, speed, incline) {
  ...
  if (workoutStarted.value) {
    try {
      await api.post('/api/v/rise/workout/complete-set', {
        session_id: sessionId.value,
        exercise_index: exIndex,
        set_number: setIndex + 1,
        exercise_name: exName(exercises.value[exIndex]),
...
async function switchWeek(week) {
  ...
  loading.value = true;
  try {
    const response = await api.get(`/api/v/rise/workout/1?week=${week}`);
...
function dismissTutorial() {
  ...
  try {
    api.post('/api/v/rise/workout/dismiss-tutorial');
  } catch (err) { /* silent */ }
}
...
async function fetchWorkout() {
  ...
  try {
    ...
    const url = dayParam ? `/api/v/rise/workout/${dayParam}` : '/api/v/rise/workout/1';
...
async function startWorkout() {
  ...
  startTimer();
  try {
    const response = await api.post('/api/v/rise/workout/start', {
...
async function finishWorkout() {
  ...
  clearRestTimer();
  try {
    const response = await api.post('/api/v/rise/workout/finish', {
      session_id: sessionId.value,
      elapsed: elapsed.value,
      set_data: setData.value,
    });
    const sid = response.data.session_id || sessionId.value;
    router.push({ name: 'rise-workout-summary', params: { sessionId: sid } });
  } catch (err) {
    router.push({ name: 'rise-workout-summary', params: { sessionId: sessionId.value || 'latest' } });
  } finally {
    saving.value = false;
  }
}
...
async function abandonWorkout() {
  ...
  clearRestTimer();
  try {
    await api.post('/api/v/rise/workout/abandon', {
      session_id: sessionId.value,
    });
  } catch (err) { /* silent */ }
  finally {
  ...
  <RiseLayout>
    <div class="min-h-screen" :class="{ 'pb-[280px]': workoutStarted }">
      ...
      <template v-else>
        ...
        >
          <div v-if="showRestTimer" class="fixed bottom-36 left-1/2 -translate-x-1/2 z-[60] rounded-2xl border border-wc-accent/30 bg-wc-bg/95 backdrop-blur-xl px-6 py-4 shadow-2xl min-w-[200px]">
